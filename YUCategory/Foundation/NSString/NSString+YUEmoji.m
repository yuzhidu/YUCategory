...
+ (void)yu_initializeEmojiCheatCodes {
    ...
    @synchronized(self) {
        yu_s_unicodeToCheatCodes = forwardMap;
        yu_s_cheatCodesToUnicode = [reversedMap copy];
    }
}

- (NSString *)yu_emojiCheatCodesReplacingWithUnicode {
    if (!yu_s_cheatCodesToUnicode) {
        [NSString yu_initializeEmojiCheatCodes];
    }
    
    if ([self rangeOfString:@":"].location != NSNotFound) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [yu_s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
...
- (NSString *)yu_emojiUnicodeReplacingWithCheatCodes {
    ...
    if (self.length) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [yu_s_unicodeToCheatCodes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
...
- (instancetype)yu_emojiRemoved {
    ...
    [self enumerateSubstringsInRange:NSMakeRange(0, [self length])
                             options:NSStringEnumerationByComposedCharacterSequences
                          usingBlock: ^(NSString* substring, NSRange substringRange, NSRange enclosingRange, BOOL* stop) {
                              [buffer appendString:([substring yu_isEmoji])? @"": substring];
                          }];
    return buffer;
}
@end
